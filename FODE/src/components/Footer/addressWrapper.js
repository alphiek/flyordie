...
import { Copyright, Address, Divider, SocialWrapper, Icon, Anchor } from './footerStyles'
...
const AddressWrapper = ({ data }) => (
  <React.Fragment>
   ...
   <Divider />
   <Anchor color={color.white} href={`mailto:${data.site.siteMetadata.email}`}>{data.site.siteMetadata.email}</Anchor>
   <SocialWrapper>
     ...
     <a href={`mailto:${data.site.siteMetadata.email}`}><Icon src={email} alt='Email Link'></Icon></a>
   </SocialWrapper>
   <Copyright color={color.white} href="https://rkkcreative.xyz" target="_blank" rel="noopener noreferrer">
